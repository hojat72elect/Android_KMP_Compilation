...
class TransferViewModel(val applicationContext: Application) :
    ...
    fun sendMessage(message: String): LiveData<Boolean> {
        return liveData(context = viewModelScope.coroutineContext + Dispatchers.Default) {
            ...
            try {
                ...
                clientHandshakeSocket?.use { socket ->
        ...
        return liveData(context = viewModelScope.coroutineContext + Dispatchers.IO) {
            peerIP?.let { host ->
                ...
                try {
                    ...
                    clientTransferSocket?.use { socket ->
    ...
    fun initServerConnection(filePath: String, fileSize: Long): LiveData<String> {
        return liveData(context = viewModelScope.coroutineContext + Dispatchers.IO) {
            try {
                ...
                serverTransferSocket?.use { serverSocket ->
                    isTransferInProgress = true
    ...
    private fun receiveMessageInternal(): Pair<InetAddress, String>? {
        try {
            ...
            serverHandshakeSocket?.use { serverSocket ->
                isTransferInProgress = true
                serverSocket.reuseAddress = true
                val client = serverSocket.accept()
                ObjectInputStream(client.getInputStream()).use { objectInputStream ->
