...
class Config(context: Context) : BaseConfig(context) {
    ...
    fun saveFolderGrouping(path: String, value: Int) {
        if (path.isEmpty()) {
            ...
            prefs.edit { putInt(GROUP_FOLDER_PREFIX + path.lowercase(Locale.ROOT), value) }
        }
    }
    ...
    fun removeFolderGrouping(path: String) {
        prefs.edit { remove(GROUP_FOLDER_PREFIX + path.lowercase()) }
    }
    ...
    fun saveFolderViewType(path: String, value: Int) {
        if (path.isEmpty()) {
            ...
            prefs.edit { putInt(VIEW_TYPE_PREFIX + path.lowercase(Locale.ROOT), value) }
        }
    }
    ...
    fun removeFolderViewType(path: String) {
        prefs.edit { remove(VIEW_TYPE_PREFIX + path.lowercase()) }
    }
    ...
    var showHiddenMedia: Boolean
        ...
        set(showHiddenFolders) = prefs.edit {
            putBoolean(SHOW_HIDDEN_MEDIA, showHiddenFolders)
        }

    var temporarilyShowHidden: Boolean
        get() = prefs.getBoolean(TEMPORARILY_SHOW_HIDDEN, false)
        set(temporarilyShowHidden) = prefs.edit {
            putBoolean(TEMPORARILY_SHOW_HIDDEN, temporarilyShowHidden)
        }
    ...
    var temporarilyShowExcluded: Boolean
        ...
        set(temporarilyShowExcluded) = prefs.edit {
            putBoolean(TEMPORARILY_SHOW_EXCLUDED, temporarilyShowExcluded)
        }

    var isThirdPartyIntent: Boolean
        get() = prefs.getBoolean(IS_THIRD_PARTY_INTENT, false)
        set(isThirdPartyIntent) = prefs.edit {
            putBoolean(IS_THIRD_PARTY_INTENT, isThirdPartyIntent)
        }
    ...
    var excludedFolders: MutableSet<String>
        ...
        set(excludedFolders) = prefs.edit {
    ...
    var includedFolders: MutableSet<String>
        ...
        set(includedFolders) = prefs.edit {
    ...
    var showThumbnailVideoDuration: Boolean
        ...
        set(showThumbnailVideoDuration) = prefs.edit {
            putBoolean(SHOW_THUMBNAIL_VIDEO_DURATION, showThumbnailVideoDuration)
        }
    ...
    var showThumbnailFileTypes: Boolean
        ...
        set(showThumbnailFileTypes) = prefs.edit {
            putBoolean(SHOW_THUMBNAIL_FILE_TYPES, showThumbnailFileTypes)
        }

    var markFavoriteItems: Boolean
        get() = prefs.getBoolean(MARK_FAVORITE_ITEMS, true)
        set(markFavoriteItems) = prefs.edit {
            putBoolean(MARK_FAVORITE_ITEMS, markFavoriteItems)
        }
    ...
    var fileLoadingPriority: Int
        ...
        set(fileLoadingPriority) = prefs.edit {
            putInt(FILE_LOADING_PRIORITY, fileLoadingPriority)
        }
    ...
    var openVideosOnSeparateScreen: Boolean
        ...
        set(openVideosOnSeparateScreen) = prefs.edit {
            putBoolean(OPEN_VIDEOS_ON_SEPARATE_SCREEN, openVideosOnSeparateScreen)
        }
    ...
    var allowInstantChange: Boolean
        ...
        set(allowInstantChange) = prefs.edit {
            putBoolean(ALLOW_INSTANT_CHANGE, allowInstantChange)
        }
    ...
    var deleteEmptyFolders: Boolean
        ...
        set(deleteEmptyFolders) = prefs.edit {
            putBoolean(DELETE_EMPTY_FOLDERS, deleteEmptyFolders)
        }

    var allowPhotoGestures: Boolean
        get() = prefs.getBoolean(ALLOW_PHOTO_GESTURES, false)
        set(allowPhotoGestures) = prefs.edit {
            putBoolean(ALLOW_PHOTO_GESTURES, allowPhotoGestures)
        }

    var allowVideoGestures: Boolean
        get() = prefs.getBoolean(ALLOW_VIDEO_GESTURES, true)
        set(allowVideoGestures) = prefs.edit {
            putBoolean(ALLOW_VIDEO_GESTURES, allowVideoGestures)
        }
    ...
    var slideshowMoveBackwards: Boolean
        ...
        set(slideshowMoveBackwards) = prefs.edit {
            putBoolean(SLIDESHOW_MOVE_BACKWARDS, slideshowMoveBackwards)
        }

    var slideshowAnimation: Int
        get() = prefs.getInt(SLIDESHOW_ANIMATION, SLIDESHOW_ANIMATION_SLIDE)
        set(slideshowAnimation) = prefs.edit {
            putInt(SLIDESHOW_ANIMATION, slideshowAnimation)
        }
    ...
    var lastFilePickerPath: String
        ...
        set(lastFilePickerPath) = prefs.edit {
            putString(LAST_FILEPICKER_PATH, lastFilePickerPath)
        }
    ...
    fun removeLastVideoPosition(path: String) {
        prefs.edit { remove("$LAST_VIDEO_POSITION_PREFIX${path.lowercase(Locale.ROOT)}") }
    }

    fun saveLastVideoPosition(path: String, value: Int) {
        if (path.isNotEmpty()) {
            prefs.edit { putInt("$LAST_VIDEO_POSITION_PREFIX${path.lowercase()}", value) }
        }
    }
    ...
    var visibleBottomActions: Int
        ...
        set(visibleBottomActions) = prefs.edit {
            putInt(VISIBLE_BOTTOM_ACTIONS, visibleBottomActions)
        }
    ...
    var everShownFolders: Set<String>
        ...
        set(everShownFolders) = prefs.edit {
            putStringSet(EVER_SHOWN_FOLDERS, everShownFolders)
        }
    ...
    var showRecycleBinAtFolders: Boolean
        ...
        set(showRecycleBinAtFolders) = prefs.edit {
            putBoolean(SHOW_RECYCLE_BIN_AT_FOLDERS, showRecycleBinAtFolders)
        }

    var allowZoomingImages: Boolean
        get() = prefs.getBoolean(ALLOW_ZOOMING_IMAGES, true)
        set(allowZoomingImages) = prefs.edit {
            putBoolean(ALLOW_ZOOMING_IMAGES, allowZoomingImages)
        }
    ...
    var lastEditorCropAspectRatio: Int
        ...
        set(lastEditorCropAspectRatio) = prefs.edit {
            putInt(LAST_EDITOR_CROP_ASPECT_RATIO, lastEditorCropAspectRatio)
        }
    ...
    var allowRotatingWithGestures: Boolean
        ...
        set(allowRotatingWithGestures) = prefs.edit {
            putBoolean(ALLOW_ROTATING_WITH_GESTURES, allowRotatingWithGestures)
        }
    ...
    var spamFoldersChecked: Boolean
        ...
        set(spamFoldersChecked) = prefs.edit {
            putBoolean(SPAM_FOLDERS_CHECKED, spamFoldersChecked)
        }
    ...
    var editorBrushHardness: Float
        ...
        set(editorBrushHardness) = prefs.edit {
            putFloat(EDITOR_BRUSH_HARDNESS, editorBrushHardness)
        }
    ...
    var wereFavoritesMigrated: Boolean
        ...
        set(wereFavoritesMigrated) = prefs.edit {
            putBoolean(WERE_FAVORITES_MIGRATED, wereFavoritesMigrated)
        }

    var showFolderMediaCount: Int
        get() = prefs.getInt(FOLDER_MEDIA_COUNT, FOLDER_MEDIA_CNT_LINE)
        set(showFolderMediaCount) = prefs.edit {
            putInt(FOLDER_MEDIA_COUNT, showFolderMediaCount)
        }
    ...
    var limitFolderTitle: Boolean
        ...
        set(limitFolderTitle) = prefs.edit {
            putBoolean(LIMIT_FOLDER_TITLE, limitFolderTitle)
        }
    ...
    var fileRoundedCorners: Boolean
        ...
        set(fileRoundedCorners) = prefs.edit {
            putBoolean(FILE_ROUNDED_CORNERS, fileRoundedCorners)
        }

    var customFoldersOrder: String
        get() = prefs.getString(CUSTOM_FOLDERS_ORDER, "")!!
        set(customFoldersOrder) = prefs.edit {
            putString(CUSTOM_FOLDERS_ORDER, customFoldersOrder)
        }
    ...
    var avoidShowingAllFilesPrompt: Boolean
        ...
        set(avoidShowingAllFilesPrompt) = prefs.edit {
            putBoolean(AVOID_SHOWING_ALL_FILES_PROMPT, avoidShowingAllFilesPrompt)
        }
    ...
    var searchAllFilesByDefault: Boolean
        ...
        set(searchAllFilesByDefault) = prefs.edit {
            putBoolean(SEARCH_ALL_FILES_BY_DEFAULT, searchAllFilesByDefault)
        }
    ...
    var lastExportedFavoritesFolder: String
        ...
        set(lastExportedFavoritesFolder) = prefs.edit {
            putString(LAST_EXPORTED_FAVORITES_FOLDER, lastExportedFavoritesFolder)
        }
}
